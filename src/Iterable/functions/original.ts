...
export const map: Functor_1<TIterable>['map'] =
  <a, b>(f: (_: a) => b) =>
  (fa: Iterable<a>) => ({
    [Symbol.iterator]() {
      const enum StateTag {
...
export const traverse = traverseDefaultFoldableUnfoldable({
  foldr,
  unfoldr,
} as FoldROnly_1<TIterable> & UnfoldROnly_1<TIterable>);
