...
export const apply = <a, b>(
  ff: Maybe<(_: a) => b>
): ((fa: Maybe<a>) => Maybe<b>) =>
  /*#__PURE__*/ bind(a => map((f: (_: a) => b) => f(a))(ff));

export const maybeL = <b>(f: () => b) => <a>(g: (_: a) => b) => (
  fa: Maybe<a>
): b => (fa.isNothing ? f() : g(fa.value));

export const fromMaybe = <a>(a: a): ((fa: Maybe<a>) => a) =>
  /*#__PURE__*/ maybe(a)(x => x);
...
export const foldl = <a, b>(f: (_: b) => (_: a) => b) => (
  b0: b
): ((fa: Maybe<a>) => b) => /*#__PURE__*/ maybe(b0)(f(b0));

export const foldr = <a, b>(f: (_: a) => (_: b) => b) => (
  b0: b
