...
>(
  key: key,
  ...args: args
) => (xs: unknown[]): b => (Array.prototype[key] as any).apply(xs, args);

const snocArray = <a>(xs: a[]) => (x: a) => {
  xs.push(x);
  return xs;
};
...
export const forEachRight = <a>(f: (_: a) => void) => (xs: a[]) => {
...
export const foldl: Foldable_1<TArray>['foldl'] = <a, b>(f: (_: b) => (_: a) => b) => (
  b: b
): ((xs: a[]) => b) => borrow('reduce', (acc: b, x: a) => f(acc)(x), b);
...
export const apply: Apply_1<TArray>['apply'] = <a, b>(fs: Array<(_: a) => b>) => (xs: a[]): b[] =>
...
export const foldlWithIndex = <a, b>(f: (_: number) => (_: b) => (_: a) => b) => (
  b: b
): ((xs: a[]) => b) => borrow('reduce', (acc: b, x: a, i) => f(i)(acc)(x), b);

export const foldr: Foldable_1<TArray>['foldr'] = <a, b>(f: (_: a) => (_: b) => b) => (
  b: b
): ((xs: a[]) => b) => borrow('reduceRight', (acc: b, x: a) => f(x)(acc), b);

export const foldrWithIndex = <a, b>(f: (_: number) => (_: a) => (_: b) => b) => (
  b: b
): ((xs: a[]) => b) => borrow('reduceRight', (acc: b, x: a, i) => f(i)(x)(acc), b);
