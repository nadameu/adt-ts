...
export const mapWithIndex = <a, b>(f: (_: number) => (_: a) => b) => {
  const go = (i: number) => (xs: LazyList<a>): LazyList<b> => (result = xs()) =>
...
export const take = (n: number) => <a>(list: LazyList<a>): LazyList<a> => (result = list()) =>
