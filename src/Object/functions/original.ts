...
export const map: Functor_O['map'] = <a, b>(f: (_: a) => b) => <T extends Record<keyof T, a>>(
  obj: T
): { [k in keyof T]: b } =>
  fromEntries(array.map(([k, v]: [keyof T, a]) => [k, f(v)] as [keyof T, b])(entries(obj)));

export const apply: Apply_O['apply'] = <T extends Record<keyof T, (_: any) => unknown>>(ff: T) => <
...
export const alt: Alt_O['alt'] = <T>(x: T) => <U>(
  y: U
...
export const foldMap: Foldable_O['foldMap'] = <m>(monoid: Anon<Monoid_0<m>>) => <a>(
  f: (_: a) => m
...
export const traverse: Traversable_O['traverse'] = <m extends Generic1>(
