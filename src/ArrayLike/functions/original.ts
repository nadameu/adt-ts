...
export const foldl: Foldable_1<TArrayLike>['foldl'] = <a, b>(f: (_: b) => (_: a) => b) => (
  b: b
...
export const foldr: Foldable_1<TArrayLike>['foldr'] = <a, b>(f: (_: a) => (_: b) => b) => (
  b: b
...
export const partitionMap: Filterable_1<TArrayLike>['partitionMap'] = <a, b, c>(
  f: (_: a) => Either<b, c>
) => (as: ArrayLike<a>) => {
  const left: b[] = [];
  const right: c[] = [];
