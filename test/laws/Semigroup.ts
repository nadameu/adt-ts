...
import { Semigroup, Semigroup1 } from '../../src/typeclasses/Semigroup';

export const makeSemigroupLaws = <a>(semigroup: Semigroup<a>) => (eq: Eq<a>) => (
  arb: jsc.Arbitrary<a>
) => {
  return {
    associativity: (): void =>
      jsc.assertForall(arb, arb, arb, (x, y, z) =>
        eq.eq(
          semigroup.append(semigroup.append(x, y), z),
