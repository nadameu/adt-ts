...
test('Generate type description (1)', () => {
  ...
  const defs = ends.map(
    pipe(
      ...
      range => {
        ...
        const fns = pairs.map(([a, b], i) => `f${i}: (_: ${a}) => Type1<f, ${b}>`);
        const first = range[0];
        const last = range[range.length - 1];
        return fns.length === 0
          ? `(): <${letters}>(_: ${first}) => Type1<f, ${last}>;`
          : `<${letters}>(${fns.join(', ')}): (_: ${first}) => Type1<f, ${last}>;`;
      }
    )
  );
  const z = `export interface PipeKleisli_1<f extends Generic1> {\n${defs
    .map(x => `  ${x}`)
    .join('\n')}\n}`;
...
test('Generate type description (2)', () => {
  const ends = A.range(-1)(19);
  const defs = ends.map(
    pipe(
      A.range(-1),
      ...
      range => {
        range[0] = 'b';
        if (range.length > 1) range[range.length - 1] = 'c';
        const letters = range.join(', ');
        const pairs = Array.from(range)
          .map((a, i, as) => [a, as[i + 1]] as [string, string | undefined])
          .filter((x): x is [string, string] => x[1] !== undefined);
        const fns = pairs.map(([a, b], i) => `f${i}: (_: ${a}) => Type2<f, a, ${b}>`);
        const first = range[0];
        const last = range[range.length - 1];
        return fns.length === 0
          ? `(): <a, ${letters}>(_: ${first}) => Type2<f, a, ${last}>;`
          : `<a, ${letters}>(${fns.join(', ')}): (_: ${first}) => Type2<f, a, ${last}>;`;
      }
    )
  );
  const z = `export interface PipeKleisli_2<f extends Generic2> {\n${defs
    .map(x => `  ${x}`)
    .join('\n')}\n}`;
  expect(z).toMatchInlineSnapshot(`
